...
Client.prototype._invoke = function(method, args, location, callback, options, extraHeaders) {
  ...
  if (this.streamAllowed && typeof self.httpClient.requestStream === 'function') {
    ...
    req.on('response', function (response) {
      ...
      response.pipe(concatStream(function (buffer) {
        const body = sliceBuffer(buffer);
        const xmlResponse = body[0].toString();

        self.lastResponse = xmlResponse;
        self.lastResponseHeaders = response && response.headers;
        self.lastElapsedTime = Date.now() - startTime;
        self.emit('response', body, response, eid);

        const obj = self.wsdl.xmlToObject(xmlResponse);
...
// TODO refactor to handle general attachments
function sliceBuffer(buffer){
  const [xml1, from1, to1] = sliceBufferByString(buffer, '<soap', '\r\n');
  const [pdf2, from2, to2] = sliceBufferByString(buffer, '%PDF', '%%EOF', to1);
  const [pdf3, from3, to3] = sliceBufferByString(buffer, '%PDF', '%%EOF', to2);
  return [xml1, pdf2, pdf3];
}

function sliceBufferByString(buffer, fromString, toString, index = 0){
  const from = buffer.indexOf(fromString, index);
  const to = buffer.indexOf(toString, from) + toString.length;
  return [buffer.slice(from, to), from, to];
}
