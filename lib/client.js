  ...
  if (this.streamAllowed && typeof self.httpClient.requestStream === 'function') {
    ...
    req.on('response', function (response) {
      ...
      response.pipe(concatStream(function (buffer) {
        const xmlResponse = sliceBufferByString(buffer, '<soap', '\r\n').toString();
        const pdfBuffer = sliceBufferByString(buffer, '%PDF', '%%EOF');
        const body = [xmlResponse, pdfBuffer];
...
function sliceBufferByString(buffer, fromString, toString){
  const from = buffer.indexOf(fromString);
